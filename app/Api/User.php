...

{
    ...
    protected $fillable = [
        ...
        'indvCustId','indvLsCustId', 'indvCustName', 'indvCustEmail','indvCustMobile','indvCustPassword','gender','indvCustStatus','authUserId','auth','authId','socialImage','createdOn','updatedOn',
        'sessionToken','web_player_id','referralCode','referredBy','points'
    ...
    public function getUserTypeByRefer($referCode){
        $userType = "";
        
        $user = DB::table('individual_user_master')->where('referralCode','=',$referCode)->first();
        if(count($user) > 0)
            return $user->userType;
        else {
            $corpUser = DB::table('corporate_user_master')->where('referralCode','=',$referCode)->first();
            if(count($corpUser) > 0)
            return $corpUser->userType;
            else 
            return $userType;
        }
