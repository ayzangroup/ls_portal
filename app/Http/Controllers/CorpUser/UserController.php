...

{
...

  {
         ...
         return view::make('corpuser.index',compact('services'));
...

  {
         ...
         return view('corpuser.myprofile');
...

  {
        ...
        $order=Orders::where('userId',$user->corpCustId)->where('userType','corp')->get();
        $order = DB::table('order_master')
            ...
            ->where('order_master.userId', $user->corpCustId)
            ...
            ->get();
        $save_order= Orders::where('status',1)->where('userId', $user->corpCustId)
            ->where('userType',$user->userType)->get();
        foreach ($save_order as $saved_orders) 
        {
          $saved_orders->order_items=$saved_orders->itemDetails;
        }
        $fav_order=Orders::where('isFavorite',1)->where('userId', $user->corpCustId)
            ->where('userType',$user->userType)->get();
        foreach ($fav_order as $fav_orders) 
        {
         ...
         return view('corpuser.dashboard',compact('order','save_order','fav_order'));
...

	{
        ...
        return view::make('corpuser.price_calculator',compact('services','categories','price','categoryid'));
...

	{
        $date=date('Y-m-d');
        $user=Auth::guard('corpuser')->user();
        $order=Orders::where('userId',$user->corpCustId)->where('userType',$user->userType)->get();
        ...
        $past_order= DB::table('order_master')
            ...
            ->orderBy('orderId','desc')
            ->get();
        $cancel_order= Orders::where('status',8)->where('userId', $user->corpCustId)
            ->where('userType',$user->userType)->orderBy('orderId','desc')->get();

        $save_order= Orders::where('status','<',4)->where('userId', $user->corpCustId)
            ->where('userType',$user->userType)->orderBy('orderId','desc')->get();
    
         return view('corpuser.order_history',compact('order','past_order','cancel_order','save_order'));
	...
	{      
        ...
        $order->deliverpincode=$order->deliverId->deliveraddress->pincode;
         ...
         return view('corpuser.track',compact('order'));
    ...
    public function getlocation(Request $request)
    {
      $lat = 28.577077;
      $lng = 77.265937;
      return view('corpuser.location',compact('lat','lng'));      
    }
...

	{
       ...
       $date=date('Y-m-d');
        ...
        if(count($order)!='0' || count($past_order)!='0')
        {
        ...
        return view('corpuser.calender',compact('events','services'));
...

	{
          ...
          $offers=Offer::where('status','1')->where('userType','corp')->get();;

         return view::make('corpuser.offer',compact('offers'));
         ...
         $faq_data=Faq::all();
          ...
          $title_name=FaqTitle::orderBy('id','asc')->get();
             ...
             if(count($faq_data)!='0')       
              ...
              return view::make('corpuser.faq',compact('data','title_name'));
...

	{
        ...
        $update=NotificationUser::where('objectId',$user->corpCustId)->update(['isViewed'=>'1']);   

        $address=Address::where('objectId',$user->corpCustId)->where('objectType',$user->userType)->get();

        return view::make('corpuser.setting',compact('address'));
...

	{
         ...
         return view::make('corpuser.help',compact('data'));
...

  {
        ...
        return view::make('corpuser.add_item',compact('categories','price','categoryid'));
   ...
   public function schedule(Request $request)

    {
        ...
        $address=Address::where('objectId',$user->corpCustId)->where('objectType',$user->userType)->get();
         return view('corpuser.schedule',compact('address','order'));
    ...
    public function add_schedule(Request $request)
    {
        ...
        if($count=='0')
        {
            ...
            $insert->userId=$user->corpCustId;
            $insert->userType=$user->userType;
            ...
            if($insert_data)
            {
              ...
              $deliver->customerId=$user->corpCustId;
              $deliver->customerType=$user->userType;
              ...
              if($deliver_insert)
              {
                 $data=Orders::where('orderId',$request->orderId)->where('userId',$user->corpCustId)->where('userType',$user->userType)->update([
      ...
      else
      {
            ...
            if($update_data)
            {
              ...
              $deliver_update->customerId=$user->corpCustId;
              $deliver_update->customerType=$user->userType;
...

  {
        ...
        $services=Services::all();
        return view('corpuser.review_order',compact('order','services'));
...

  {
        ...
        Session::push('cart',$order);
         return view('corpuser.payment',compact('order'));
     ...
     public function voucher_payment(Request $request)
    {
        ...
        if($match)
        {          
            return view::make('corpuser.voucher_payment',compact('match'));
...

  {
    ...
    $data[] = [              
           ...
           $data1=json_encode(['userType'=> 'corp','address'=>$data,'userId' =>  $request->id]);
...

                  {
                    ...
                    {
                      return redirect()->route('corpuser.setting');
                    }
...

    {
...

      {
        ...
        return view::make('corpuser.edit_address',compact('address'));
...

    {
...

      {
          ...
          $update=CorpUser::find($request->id);

          $update->concerPersonMobile=$request->phone;

          $save=$update->save();

          return redirect()->route('corpuser.myprofile')->with('error_code','6')->with('message','Phone number update successfully!')->with('status','Profile Update');
...

      {
          ...
          return redirect()->route('corpuser.myprofile')->with('error_code','6')->with('message','Phone number update successfully!')->with('status','Profile Update Failed');
...

    {
...

      {
          ...
          $update->corpPassword=Hash::make($request->password);

          $save=$update->save();

          return redirect()->route('corpuser.myprofile')->with('error_code','6')->with('message','Password update successfully!')->with('status','Profile Update');
...

      {
          ...
          return redirect()->route('corpuser.myprofile')->with('error_code','6')->with('message','Password is not match!')->with('status','Profile Update Failed');
...

    {
        ...
        $facebook=Share::load($url)->facebook();
    ...
    public function update_item(Request $request)
          ...
          public function updateItem(Request $request)
          {
              ...
              if(Auth::guard('user')->user() || Auth::guard('corpuser')->user() )
              {
                ...
                {
                  return view::make('corpuser.update_item',compact('data','data1')); 
                }
            }
            else
                  {
          ...
          public function saveItem(Request $request)
          {
              ...
              if(Auth::guard('user')->user() || Auth::guard('corpuser')->user() )
              {
                if(isset($request->orderStr) && $request->orderStr != "")
                {
                  ...
                  if($user=Auth::guard('corpuser')->user())
                  {
                    $save=Orders::where('orderId',$userSave->orderId)->update(['userId'=>$user->corpCustId,'userType'=>'corp']);
                ...
                else
                {
            ...
            else
                  {
                    ...
                    Session::forget('orderstr');
                    Session::put('orderstr',$orderStr);
                    $url=url('corpuser/order_history');
                    return $url;
                  }
          ...
          public function coupoan(Request $request)
        {
            ...
            return view::make('corpuser.coupoan',compact('coupoan'));
