...
<div class="common-background price-calculator">
        ...
        <div class="tabs-services-box">
            ...
            <ul class="nav nav-tabs nav-pills tabs-services" id="pills-tab" role="tablist">
            @php
            $i=1;
            @endphp
        ...
        <div class="row">
                            ...
                            <table class="table table-bordered table table-striped">                            
                                ...
                                @endphp
                                 ...
                                 @foreach($price as $price_list)
                                   ...
                                   @if(in_array($price_list->itemId,$itemsArr))
                                    @foreach($order_items as $items)
                                    @if($items->itemId == $price_list->itemId)
                                    @php
                                   
                                        $val = "";
                                        $val = $items->quantity. ",". $items->itemName. ",".$items->rate.",".$items->itemId.",".$price_list->category_id; 
                                        
                                    @endphp
                                    <tr>

                                        <td>{{ucfirst($items->itemName)}}</td>

                                        <td>Rs. {{ceil($items->rate)}}</td>
                                        ...
                                        <div class="item-from " id="fieldId{{$j}}" style="display:flex;">
                                            ...
                                            <input type="text" name="quant[1]{{$j}}" id="code{{$j}}" value="{{$items->quantity}}" min="0" max="10">
                                            <input type="hidden" name="item" id="item{{$j}}" value="{{$items->itemName}}">
                                            <input type="hidden" name="price" id="price{{$j}}" value="{{$items->rate}}">
                                            <input type="hidden" name="itemId" id="itemId{{$j}}" value="{{$items->itemId}}">
                                        ...
                                        <input type="hidden" name="hdItemVal" id="hdItemVal{{$j}}" value="{{$val}}" />
                                    </tr>
                                    @endif
                                    @endforeach
                                @else
                                ...
                                @endif
                                    ...
                                    @endforeach
                                        ...
                                        <input type="hidden" id="url1" name="url1" value="{{Request::segment(1)}}" />
                                    @else
                                    <tr>
            ...
            @endphp
            @endforeach
            <input type="hidden" name="repeat_url" id="repeat_url" value="{{Request::segment(2)}}">
  ...
  $(document).ready(function(){
    $(document).on('click','.plus-minus-btn',function(e){
        ...
        $.ajax({
          url:path+"/add_item",
          method:"POST",
          data:{"_token":"{{ csrf_token() }}","codeValue":codeValue,'repeat_url':repeat_url,"url1":url1},
        ...
        // $(document).on('load','#ajax',function(e){
        //     var codeValue = "";
        //     var itemVal = $("input[name='hdItemVal']");
        //     for(var i = 0;i < itemVal.length;i++)
        //     {
        ...
        $(document).on('click','#orderSave',function(e){
            ...
            if(chk){
                ...
                $.ajax({
                    url:path+"/save-item",
                    method:"POST",
                    data:{"_token":"{{ csrf_token() }}","orderStr":orderStr,"url1":url1},
                    beforeSend: function() {
                        $("#loading-image").show();
                    },
                    success:function(data){
        ...
        var ajaxLoad = function(){
            ...
            $.ajax({
                url:path+"/add_item",
                method:"POST",
                data:{"_token":"{{ csrf_token() }}","codeValue":codeValue,"repeat_url":repeat_url},
                success:function(data){
                    $("#ajax").html(data);
                }
            });
